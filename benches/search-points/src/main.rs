...
use criterion::*;
use solvio_client::prelude::*;
use solvio_client::solvio::vectors_config;
use rand::prelude::*;
...
impl Config {
    ...
    pub async fn client(&self) -> Result<SolvioClient> {
        let config = SolvioClientConfig {
            uri: self.uri.clone(),
        ...
        let client = SolvioClient::new(Some(config)).await?;
...
fn client(config: &Config, runtime: &Runtime) -> Result<Arc<SolvioClient>> {
    let client = runtime.block_on(config.client())?;
    Ok(Arc::new(client))
}

fn vector_size(config: &Config, runtime: &Runtime, client: &SolvioClient) -> Result<usize> {
