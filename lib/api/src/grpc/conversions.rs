...
use segment::data_types::vectors::{DenseVector, MultiDenseVector, NamedMultiDenseVector};
...
use super::solvio::{
    ...
    GroupId, MaxSim, MultiVectorConfig, OrderBy, Range, SparseIndices, StartFrom,
};
...
use crate::grpc::solvio::vectors::VectorsOptions;
    ...
    fn from(vector: segment::data_types::vectors::Vector) -> Self {
        match vector {
            ...
            segment::data_types::vectors::Vector::MultiDense(vector) => {
                let tokens_count = vector.multi_vectors().count() as u32;
                Self {
                    data: vector.inner_vector,
                    indices: None,
...
impl TryFrom<Vector> for segment::data_types::vectors::Vector {
    ...
    fn try_from(vector: Vector) -> Result<Self, Self::Error> {
        ...
        // multivector
        if let Some(tokens_count) = vector.tokens_count {
            // TODO prevent division by zero
            let dim = vector.data.len() / tokens_count as usize;
    ...
    fn from(key: segment::data_types::groups::GroupId) -> Self {
        match key {
            segment::data_types::groups::GroupId::String(str) => Self {
                kind: Some(crate::grpc::solvio::group_id::Kind::StringValue(str)),
            },
            segment::data_types::groups::GroupId::NumberU64(n) => Self {
                kind: Some(crate::grpc::solvio::group_id::Kind::UnsignedValue(n)),
            },
            segment::data_types::groups::GroupId::NumberI64(n) => Self {
                kind: Some(crate::grpc::solvio::group_id::Kind::IntegerValue(n)),
            },
        }
    }
}
        ...
        match vectors.vectors_options {
            Some(vectors_options) => Ok(match vectors_options {
                VectorsOptions::Vector(vector) => {
                    if vector.tokens_count.is_some() {
                        return Err(Status::invalid_argument(
        ...
        ScalarQuantization {
            r#type: match config.r#type {
                segment::types::ScalarType::Int8 => {
                    crate::grpc::solvio::QuantizationType::Int8 as i32
                }
            },
    ...
    fn from(value: segment::types::QuantizationConfig) -> Self {
        match value {
            segment::types::QuantizationConfig::Scalar(scalar) => Self {
                quantization: Some(super::solvio::quantization_config::Quantization::Scalar(
                    scalar.into(),
                )),
            },
            segment::types::QuantizationConfig::Product(product) => Self {
                quantization: Some(super::solvio::quantization_config::Quantization::Product(
                    product.into(),
                )),
            },
            segment::types::QuantizationConfig::Binary(binary) => Self {
                quantization: Some(super::solvio::quantization_config::Quantization::Binary(
                    binary.into(),
                )),
            },
        }
...
impl TryFrom<QuantizationConfig> for segment::types::QuantizationConfig {
    ...
    fn try_from(value: QuantizationConfig) -> Result<Self, Self::Error> {
        ...
        match value {
            super::solvio::quantization_config::Quantization::Scalar(config) => Ok(
                segment::types::QuantizationConfig::Scalar(config.try_into()?),
            ),
            super::solvio::quantization_config::Quantization::Product(config) => Ok(
                segment::types::QuantizationConfig::Product(config.try_into()?),
            ),
            super::solvio::quantization_config::Quantization::Binary(config) => Ok(
...
impl From<segment::types::MultiVectorConfig> for MultiVectorConfig {
    fn from(value: segment::types::MultiVectorConfig) -> Self {
        match value {
            segment::types::MultiVectorConfig::MaxSim(max_sim) => Self {
                multivector: Some(super::solvio::multi_vector_config::Multivector::MaxSim(
                    max_sim.into(),
                )),
            },
        }
...
impl TryFrom<MultiVectorConfig> for segment::types::MultiVectorConfig {
    ...
    fn try_from(value: MultiVectorConfig) -> Result<Self, Self::Error> {
        let value = value
            .multivector
            .ok_or_else(|| Status::invalid_argument("Unable to convert multivector config"))?;
        match value {
            super::solvio::multi_vector_config::Multivector::MaxSim(config) => Ok(
                segment::types::MultiVectorConfig::MaxSim(config.try_into()?),
            ),
        }
    }
}

impl From<segment::types::MaxSimConfig> for MaxSim {
    fn from(_value: segment::types::MaxSimConfig) -> Self {
        Self {}
    }
}

impl TryFrom<MaxSim> for segment::types::MaxSimConfig {
    type Error = Status;

    fn try_from(_value: MaxSim) -> Result<Self, Self::Error> {
        Ok(segment::types::MaxSimConfig {})
    }
}
...
pub fn into_named_vector_struct(
    ...
    if let Some(indices) = indices {
        return Ok(NamedVectorStruct::Sparse(NamedSparseVector {
            name: vector_name
    ...
    // multivector
    if let Some(tokens_count) = tokens_count {
        // TODO(colbert) division by zero
        let dim = vector.len() / tokens_count as usize;
        let multi_vector = MultiDenseVector::new(vector, dim);
        return Ok(NamedVectorStruct::MultiDense(NamedMultiDenseVector {
            name: vector_name
                .ok_or_else(|| Status::invalid_argument("Multivector must have a name"))?,
            vector: multi_vector,
        }));
    }
    ...
    if let Some(vector_name) = vector_name {
        Ok(NamedVectorStruct::Dense(NamedVector {
            name: vector_name,
            vector,
        }))
    } else {
        Ok(NamedVectorStruct::Default(vector))
    }
}
