...
use crate::grpc::solvio::solvio_client::SolvioClient;
use crate::grpc::solvio::HealthCheckRequest;
...
/// More info: <https://github.com/solvio/solvio/issues/1907>
    ...
    async fn check_connectability(&self, uri: &Uri) -> HealthCheckError {
        loop {
            ...
            match channel {
                ...
                Some(Ok(channel)) => {
                    let mut client = SolvioClient::new(channel.item().clone());
