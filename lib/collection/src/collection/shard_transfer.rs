    ...
    pub fn initiate_shard_transfer(
        ...
        async move {
            ...
            if replica_set.is_dummy().await {
                ...
                // 1. Solvio is in recovery mode, and user intentionally triggered a transfer
                // 2. Shard is dirty (shard initializing flag), and Solvio triggered a transfer to recover from Dead state after an update fails
                //
                ...
                if tokio::fs::try_exists(&shard_flag).await.is_ok() {
                    ...
                    // because if transfer fails in between, Solvio will retry it.
