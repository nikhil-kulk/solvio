...
impl From<api::grpc::solvio::Vector> for RecommendExample {
        ...
        Ok(Self::Struct(OrderBy {
            ...
            start_from: value.start_from.and_then(|value| {
                value.value.map(|v| match v {
                    api::grpc::solvio::start_from::Value::Float(float) => float,
                    api::grpc::solvio::start_from::Value::Integer(int) => int as _,
                })
            }),
        }))
    }
