...
impl<'a> From<CollectionDissimilaritySearchRequest<'a>>
    for api::grpc::solvio::DissimilaritySearchPoints
{
    fn from(value: CollectionDissimilaritySearchRequest<'a>) -> Self {
        let (collection_id, request) = value.0;

        api::grpc::solvio::DissimilaritySearchPoints {
            collection_name: collection_id,
            vector: request.vector.get_vector().clone(),
            filter: request.filter.clone().map(|f| f.into()),
...
impl TryFrom<api::grpc::solvio::DissimilaritySearchPoints> for DissimilaritySearchRequest {
    type Error = Status;

    fn try_from(value: api::grpc::solvio::DissimilaritySearchPoints) -> Result<Self, Self::Error> {
