...
use api::grpc::solvio::update_collection_cluster_setup_request::Operation as ClusterOperationsPb;
use api::grpc::solvio::QuantizationType;
...
impl TryFrom<api::grpc::solvio::update_vectors_config::Config> for UpdateVectorsConfig {
    type Error = Status;

    fn try_from(
        value: api::grpc::solvio::update_vectors_config::Config,
    ) -> Result<Self, Self::Error> {
        Ok(match value {
            api::grpc::solvio::update_vectors_config::Config::Params(vector_params) => {
                UpdateVectorsConfig::Single(vector_params.try_into()?)
            }
            api::grpc::solvio::update_vectors_config::Config::ParamsMap(vectors_params) => {
                let mut params_map = BTreeMap::new();
                for (name, params) in vectors_params.map {
                    params_map.insert(name, params.try_into()?);
                }
...
impl TryFrom<api::grpc::solvio::UpdateVectorParams> for UpdateVectorParams {
    type Error = Status;

    fn try_from(vector_params: api::grpc::solvio::UpdateVectorParams) -> Result<Self, Self::Error> {
        Ok(Self {
            hnsw_config: vector_params.hnsw_config.map(Into::into),
        })
    }
        ...
        api::grpc::solvio::quantization_config::Quantization::Product(config) => {
            Ok(QuantizationConfig::Product(ProductQuantization {
                product: ProductQuantizationConfig {
                    compression: match api::grpc::solvio::CompressionRatio::from_i32(
                        config.compression,
                    ) {
                        None => {
                            return Err(format!(
                        ...
                        Some(api::grpc::solvio::CompressionRatio::X4) => CompressionRatio::X4,
                        Some(api::grpc::solvio::CompressionRatio::X8) => CompressionRatio::X8,
                        Some(api::grpc::solvio::CompressionRatio::X16) => CompressionRatio::X16,
                        Some(api::grpc::solvio::CompressionRatio::X32) => CompressionRatio::X32,
                        Some(api::grpc::solvio::CompressionRatio::X64) => CompressionRatio::X64,
                    },
                    always_ram: config.always_ram,
                },
            }))
...
impl TryFrom<api::grpc::solvio::SearchPointGroups> for SearchGroupsRequest {
    type Error = Status;
...
impl TryFrom<api::grpc::solvio::RecommendPointGroups> for RecommendGroupsRequest {
    type Error = Status;
...
impl From<api::grpc::solvio::MoveShard> for MoveShard {
    fn from(value: api::grpc::solvio::MoveShard) -> Self {
        Self {
            shard_id: value.shard_id,
