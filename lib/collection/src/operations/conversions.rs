...
use api::grpc::solvio::{CreateShardKey, Vectors};
    ...
    fn try_from(value: api::grpc::solvio::QuantizationConfigDiff) -> Result<Self, Self::Error> {
        match value.quantization {
            ...
            Some(quantization) => match quantization {
                ...
                Quantization::Rabitq(rabitq) => Ok(Self::RaBitQ(rabitq.try_into()?)),
    ...
    fn from(value: CollectionInfo) -> Self {
        ...
        api::grpc::solvio::CollectionInfo {
            ...
            config: Some(api::grpc::solvio::CollectionConfig {
                ...
                wal_config: config
                    .wal_config
                    .map(|wal_config| api::grpc::solvio::WalConfigDiff {
...
impl From<RecordInternal> for api::grpc::solvio::RetrievedPoint {
    fn from(record: RecordInternal) -> Self {
        ...
        Self {
            ...
            vectors: vectors.map(api::grpc::solvio::VectorsOutput::from),
...
fn grpc_to_segment_quantization_config(
    ...
    match quantization {
        ...
        api::grpc::solvio::quantization_config::Quantization::Rabitq(config) => {
            Ok(QuantizationConfig::RaBitQ(config.try_into()?))
        }
    }
}
