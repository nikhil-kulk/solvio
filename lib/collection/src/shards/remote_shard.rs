...
use api::grpc::solvio::solvio_client::SolvioClient;
use api::grpc::solvio::shard_snapshot_location::Location;
use api::grpc::solvio::shard_snapshots_client::ShardSnapshotsClient;
use api::grpc::solvio::{
...
impl RemoteShard {
    ...
    async fn with_solvio_client<T, Fut: Future<Output = Result<T, Status>>>(
        &self,
        f: impl Fn(SolvioClient<InterceptedService<Channel, AddTimeout>>) -> Fut,
    ) -> CollectionResult<T> {
        let current_address = self.current_address()?;
        self.channel_service
            .channel_pool
            .with_channel(&current_address, |channel| {
                let client = SolvioClient::new(channel);
                f(client)
            })
            .await
            .map_err(|err| err.into())
        ...
        let res = self
            .with_collections_client(|mut client| async move {
                let mut wait_for_shard_request = tonic::Request::new(WaitForShardStateRequest {
                    collection_name: collection_name.into(),
                    shard_id,
                    state: api::grpc::solvio::ReplicaState::from(state) as i32,
    ...
    pub async fn health_check(&self) -> CollectionResult<()> {
        let _ = self
            .with_solvio_client(|mut client| async move {
        ...
        let response = self
            .with_points_client(|mut client| async move {
                let request = &FacetCountsInternal {
                    collection_name: self.collection_id.clone(),
                    key: key.to_string(),
                    filter: filter.clone().map(api::grpc::solvio::Filter::from),
                    limit: *limit as u64,
                    exact: *exact,
                    shard_id: self.id,
                    timeout: processed_timeout.map(|t| t.as_secs()),
