...
use api::grpc::solvio::collections_internal_client::CollectionsInternalClient;
use api::grpc::solvio::points_internal_client::PointsInternalClient;
use api::grpc::solvio::{
    CollectionOperationResponse, CountPoints, CountPointsInternal,
    DissimilaritySearchBatchPointsInternal, GetCollectionInfoRequest,
...
use crate::operations::types::{
    ...
    DissimilaritySearchRequest, DissimilaritySearchRequestBatch, PointRequest, Record,
...
pub struct CollectionDissimilaritySearchRequest<'a>(
    pub(crate) (CollectionId, &'a DissimilaritySearchRequest),
);
...
impl ShardOperation for RemoteShard {
    ...
    async fn dissimilarity_search(
        ...
        let dissimilarity_search_points = batch_request
            .searches
            .iter()
            .map(|s| CollectionDissimilaritySearchRequest((self.collection_id.clone(), s)).into())
            .collect();

        let request = &DissimilaritySearchBatchPointsInternal {
            collection_name: self.collection_id.clone(),
        ...
        let search_batch_response = self
            .with_points_client(|mut client| async move {
                client
                    .dissimilarity_search_batch(tonic::Request::new(request.clone()))
                    .await
            })
            .await?
            .into_inner();
