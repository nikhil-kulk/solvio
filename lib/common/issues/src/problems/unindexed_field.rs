...
impl Issue for UnindexedField {
    ...
    fn solution(&self) -> Solution {
        ...
        Solution::Immediate(ImmediateSolution {
                    message: format!(
                        "Create an index on field '{}' of type '{}' in collection '{}'. Check the documentation for more details: https://solvio.tech/documentation/concepts/indexing/#payload-index",
                        self.field_name, self.field_type, self.collection
                    ),
                    action: Action {
                        method: Method::POST,
