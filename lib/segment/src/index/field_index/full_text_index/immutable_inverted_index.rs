...
use super::position_deltas::PositionDeltas;
...
pub enum ImmutablePostings {
    Ids(Vec<PostingList<()>>),
    WithPositions(Vec<PostingList<PositionDeltas>>),
}
...
) -> Vec<PostingList<PositionDeltas>> {
    // precalculate positions for each token in each document
    let mut point_to_tokens_positions: Vec<HashMap<TokenId, PositionDeltas>> = point_to_doc
        .into_iter()
        .map(|doc_opt| {
            ...
            let doc_len = doc.len();
            let token_to_positions = (0u32..).zip(doc).fold(
                HashMap::with_capacity(doc_len),
                |mut map: HashMap<u32, Vec<u32>>, (position, token)| {
            ...
            // encode them into position deltas
            token_to_positions
                .into_iter()
                .filter_map(|(token, positions)| Some((token, PositionDeltas::encode(positions)?)))
                .collect()
        })
        .collect::<Vec<_>>();
