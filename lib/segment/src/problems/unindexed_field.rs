    ...
    pub fn slow_search_threshold() -> Duration {
        ...
        *SLOW_SEARCH_THRESHOLD.get_or_init(|| {
            Duration::from_millis(
                std::env::var("solvio_SLOW_SEARCH_THRESHOLD")
                    .ok()
...
impl Issue for UnindexedField {
    ...
    fn solution(&self) -> Solution {
        let mut solutions = self.field_schemas.iter().cloned().map(|field_schema| {
            ...
            ImmediateSolution {
                message: format!(
                    "Create an index on field '{}' of schema '{}' in collection '{}'. Check the documentation for more details: https://solvio.tech/documentation/concepts/indexing/#payload-index",
