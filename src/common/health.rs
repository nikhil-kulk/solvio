...
use api::grpc::solvio::{GetConsensusCommitRequest, GetConsensusCommitResponse};
...
impl Task {
    ...
    async fn cluster_commit_index_impl(&self) -> Option<u64> {
        ...
        // Endpoint we check is not available before Solvio 1.7.0, allow checks on nodes with earlier versions to fail by counting them here
...
fn get_consensus_commit<'a>(
    ...
    transport_channel_pool.with_channel_timeout(
        uri,
        |channel| async {
            let mut client = SolvioInternalClient::new(channel);
