...
use anyhow::Context;
use api::grpc::solvio::raft_client::RaftClient;
use api::grpc::solvio::{PeerId, RaftMessage as GrpcRaftMessage};
...
impl Consensus {
    ...
    pub fn new(
        ...
        // They might have not been applied due to unplanned Solvio shutdown
    ...
    async fn bootstrap(
        ...
        let all_peers = client
            .add_peer_to_known(tonic::Request::new(
                api::grpc::solvio::AddPeerToKnownMessage {
                    uri,
        ...
        client
            .add_peer_as_participant(tonic::Request::new(api::grpc::solvio::PeerId { id }))
            .await
