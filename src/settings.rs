...
impl Settings {
    pub fn new(custom_config_path: Option<String>) -> Result<Self, ConfigError> {
        ...
        #[cfg(feature = "deb")]
        {
            ...
            config = config.add_source(File::with_name("/etc/solvio/config").required(false));
        }
        ...
        // E.g.: `solvio_DEBUG=1 ./target/app` would set `debug=true`
        config = config.add_source(Environment::with_prefix("solvio").separator("__"));
    ...
    pub fn tls_config_is_undefined_error() -> io::Error {
        io::Error::other("TLS config is not defined in the Solvio config file")
    }
