...
solvio_HOST = os.environ.get("solvio_HOST", "localhost")
solvio_PORT = 6333


def assert_read_only(method, path, data=None):
    assert_rw_token_success(method, f"http://{solvio_HOST}:{solvio_PORT}{path}", data)
    assert_ro_token_success(method, f"http://{solvio_HOST}:{solvio_PORT}{path}", data)


def assert_read_write(method, path, data=None):
    assert_rw_token_success(method, f"http://{solvio_HOST}:{solvio_PORT}{path}", data)
    assert_ro_token_failure(method, f"http://{solvio_HOST}:{solvio_PORT}{path}", data)
