...
curl --fail -s "http://$solvio_HOST/collections/test_collection" | jq
...
# retrieve point
curl -L -X GET "http://$solvio_HOST/collections/test_collection/points/2" \
  -H 'Content-Type: application/json' \
  --fail -s | jq

# retrieve points
curl -L -X POST "http://$solvio_HOST/collections/test_collection/points" \
  -H 'Content-Type: application/json' \
  --fail -s \
  --data-raw '{
      "ids": [1, 2]
...
# scroll points
curl -L -X POST "http://$solvio_HOST/collections/test_collection/points/scroll" \
  --fail -s \
  -H 'Content-Type: application/json' \
  --data-raw '{ "offset": 2, "limit": 2, "with_vector": true }' | jq

# change aliases
curl -L -X POST "http://$solvio_HOST/collections/aliases" \
  --fail -s \
  -H 'Content-Type: application/json' \
  --data-raw '{
      "actions": [
...
# search points
curl -L -X POST "http://$solvio_HOST/collections/test_alias/points/search" \
  -H 'Content-Type: application/json' \
  --fail -s \
  --data-raw '{
        "vector": [0.2,0.1,0.9,0.7],
...
curl -L -X POST "http://$solvio_HOST/collections/test_collection/points/delete?wait=true" \
  -H 'Content-Type: application/json' \
  --fail -s \
  --data-raw '{
    "filter": {
...
# create payload
curl -L -X POST "http://$solvio_HOST/collections/test_collection/points/payload?wait=true" \
  -H 'Content-Type: application/json' \
  --data-raw '{
...
# check payload
PAYLOAD_LEN=$(curl --fail -s "http://$solvio_HOST/collections/test_collection/points/6" | jq '.result.payload | length')
[[ "$PAYLOAD_LEN" == "1" ]] || {
...
curl -L -X POST "http://$solvio_HOST/collections/test_collection/points/payload/clear?wait=true" \
  -H 'Content-Type: application/json' \
  --fail -s \
  --data-raw '{
    "filter": {
...
# index payload
INDEXED_FIELD=$(curl --fail -s "http://$solvio_HOST/collections/test_collection" | jq '.result.payload_schema.test_payload.indexed')
[[ "$INDEXED_FIELD" == "false" ]] || {
  echo 'check failed - field should not be indexed'
  exit 1
}

curl -X PUT "http://$solvio_HOST/collections/test_collection/index?wait=true" \
  -H 'Content-Type: application/json' \
  --data-raw '{
...
curl -X DELETE "http://$solvio_HOST/collections/test_collection/index/test_payload?wait=true" \
  -H 'Content-Type: application/json' \
  --fail -s | jq
...
# delete payload
curl -L -X POST "http://$solvio_HOST/collections/test_collection/points/payload/delete?wait=true" \
  -H 'Content-Type: application/json' \
  --data-raw '{
    "keys": [ "test_payload" ],
    "points": [ 6 ]
...
curl -L -X GET "http://$solvio_HOST/collections/test_collection/points/1d675313-d3dd-4646-8b98-8052364872da" \
  -H 'Content-Type: application/json' \
  --fail -s | jq
...
curl --request POST \
  --url "http://$solvio_HOST/collections?wait=true" \
...
curl --request POST \
  --url "http://$solvio_HOST/collections/example_collection_2?wait=true" \
  --header 'Content-Type: application/json' \
  --fail -s \
  --data '{
	"upsert_points": {
