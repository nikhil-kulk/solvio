...
curl -L -X PUT  "http://$solvio_HOST/collections/test_collection/index" \
  -H 'Content-Type: application/json' "${solvio_host_headers[@]}" \
  --fail -s \
  --data-raw '{
      "field_name": "city",
...
curl --fail -s "http://$solvio_HOST/collections/test_collection" "${solvio_host_headers[@]-}" | jq
...
SAVED_POINTS_COUNT=$(curl --fail -s "http://$solvio_HOST/collections/test_collection" "${solvio_host_headers[@]}" | jq '.result.points_count')
