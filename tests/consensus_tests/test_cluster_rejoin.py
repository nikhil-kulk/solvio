...
def test_rejoin_origin_from_state(tmp_path: pathlib.Path):
    """
    This test checks that Solvio persists origin peer ID (`first_voter` field in `raft_state.json`)
...
def test_rejoin_no_origin(tmp_path: pathlib.Path):
    ...
    It simulates *earlier* behavior of Solvio (bypassing all fixes to commit/persist/recover origin
...
def test_rejoin_recover_origin(tmp_path: pathlib.Path):
    """
    This test checks that Solvio recovers origin peer ID from WAL, if origin peer was not yet
...
def rejoin_cluster_test(
    ...
    See: <https://github.com/solvio/solvio/issues/5138>
    """
