...
def check_consistency_2(peer_api_uris, loop_num='final'):
    ...
    def get_points_from_all_peers_parallel(solvio_peers, attempt_number, node_points_map):
        for node_idx, uri in enumerate(solvio_peers):
        ...
        with ProcessPoolExecutor() as executor:
            future_to_uri = {
                ...
                node_idx, uri in enumerate(solvio_peers)}
...
def test_consistency(tmp_path: pathlib.Path):
    ...
    # peer_api_uris, peer_dirs, bootstrap_uri = start_cluster(tmp_path, N_PEERS, extra_env={"solvio__STORAGE__SHARD_TRANSFER_METHOD": "snapshot"})
    grpc_uris = []
