...
def test_dirty_shard_handling_with_active_replicas(tmp_path: pathlib.Path, transfer_method: str):
    ...
    peer_api_uris, peer_dirs, bootstrap_uri = start_cluster(
        tmp_path,
        N_PEERS,
        extra_env={"solvio__STORAGE__SHARD_TRANSFER_METHOD": transfer_method },
    )
    ...
    # this can happen in practice when a node is killed while shard directory was being moved from /solvio/snapshots to /solvio/storage
    # the initializing flag is created but never deleted in such cases, when Solvio restarts it considers it as dirty shard and tries to recover it
    flag_path = shard_initializing_flag(peer_dirs[-1], COLLECTION_NAME, 0)
    ...
    # Solvio loads a dummy shard (since handle_collection_load_errors=true) so we get 0 points instead of 1000 and test fails
    # Ideally Solvio should try to recover this shard automatically using other replicas
