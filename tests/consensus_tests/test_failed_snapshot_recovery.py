...
def test_corrupted_snapshot_recovery(tmp_path: pathlib.Path):
    ...
    try:
        ...
        wait_for(is_solvio_started)
    except Exception as e:
        raise Exception(f"Solvio did not start in time after recovering corrupt snapshot, maybe it crashed: {e}")
...
def test_dirty_shard_handling_with_active_replicas(tmp_path: pathlib.Path, transfer_method: str):
    assert_project_root()
    extra_env = {
        "solvio__STORAGE__SHARD_TRANSFER_METHOD": transfer_method,
        "solvio__STORAGE__HANDLE_COLLECTION_LOAD_ERRORS": "false"
    }
