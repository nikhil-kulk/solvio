...
def test_upserts_in_recovery_mode(tmp_path: pathlib.Path):
    ...
    extra_env = {
        "solvio__STORAGE__RECOVERY_MODE": "Custom recovery msg"
    }
    p = processes.pop()
    p.kill()
    ...
    # Solvio loads a dummy shard but doesn't try to recover it from other replicas by default
    ...
    # However, Solvio will not initiate a transfer to such replicas
