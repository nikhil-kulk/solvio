...
def start_peer(peer_dir: Path, log_file: str, bootstrap_uri: str, port=None, extra_env=None, reinit=False, given_grpc_port=None, given_http_port=None) -> str:
    ...
    if reinit:
        proc = Popen([get_solvio_exec(), "--bootstrap", bootstrap_uri, "--uri", this_peer_consensus_uri, "--reinit"], env=env,
                     cwd=peer_dir, stdout=log_file)
    else:
        proc = Popen([get_solvio_exec(), "--bootstrap", bootstrap_uri, "--uri", this_peer_consensus_uri], env=env,
                 cwd=peer_dir, stdout=log_file)
...
def start_first_peer(peer_dir: Path, log_file: str, port=None, extra_env=None, reinit=False, given_grpc_port=None, given_http_port=None) -> Tuple[str, str]:
    if extra_env is None:
        extra_env = {}
    ...
    if reinit:
        proc = Popen([get_solvio_exec(), "--uri", bootstrap_uri, "--reinit"], env=env, cwd=peer_dir, stdout=log_file)
    else:
        proc = Popen([get_solvio_exec(), "--uri", bootstrap_uri], env=env, cwd=peer_dir, stdout=log_file)
    processes.append(PeerProcess(proc, http_port, grpc_port, p2p_port))
...
def check_collection_resharding_operations_count(peer_api_uri: str, collection_name: str,
    ...
    # See: <https://github.com/solvio/solvio/pull/4599>
