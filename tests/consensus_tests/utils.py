...
def get_env(p2p_port: int, grpc_port: int, http_port: int) -> Dict[str, str]:
    ...
    env["solvio__LOG_LEVEL"] = "DEBUG,raft::raft=debug"
    return env
...
def start_peer(peer_dir: Path, log_file: str, bootstrap_uri: str, port=None) -> str:
    ...
    processes.append(
        Popen([get_solvio_exec(), "--bootstrap", bootstrap_uri, "--uri", this_peer_consensus_uri], env=env,
...
def start_cluster(tmp_path, num_peers):
    ...
    # Start bootstrap
    (bootstrap_api_uri, bootstrap_uri) = start_first_peer(peer_dirs[0], "peer_0_0.log", port=10000)
