...
def get_solvio_exec() -> str:
    directory_path = os.getcwd()
    if is_coverage_mode():
        solvio_exec = directory_path + "/target/llvm-cov-target/debug/solvio"
    else:
...
def get_llvm_profile_file() -> str:
    ...
    llvm_profile_file = project_root + "/target/llvm-cov-target/solvio-consensus-tests-%m.profraw"
...
def start_peer(peer_dir: Path, log_file: str, bootstrap_uri: str, port=None, extra_env=None, reinit=False, uris_in_env=False) -> str:
    ...
    if uris_in_env:
        env["solvio_BOOTSTRAP"] = bootstrap_uri
        env["solvio_URI"] = this_peer_consensus_uri
    else:
