...
cp ./lib/api/src/grpc/{,.diff.}solvio.rs
...
if diff -Zwa ./lib/api/src/grpc/{,.diff.}solvio.rs \
...
# Cleanup
rm -f ./lib/api/src/grpc/{.diff.solvio.rs,proto/.build-trigger.proto} ./docs/grpc/.diff.docs.md
