...
export solvio__SERVICE__HOST="0.0.0.0"
export solvio__SERVICE__API_KEY="my-secret"
export solvio__SERVICE__READ_ONLY_API_KEY="my-ro-secret"
...
solvio_HOST='localhost'
until curl --output /dev/null --silent --get --fail -H 'api-key: my-ro-secret' http://$solvio_HOST:6333/collections; do
...
docker run --rm \
       -e solvio_HOST=host.docker.internal \
