...
parser.add_argument('--solvio', nargs='?', type=str, default="./target/debug/solvio", help=' solvio exec')
...
def clean_up_solvio_run(parent_folder: str = None):
...
def run_mri_and_get_timestamps(solvio_cmd, usage_output_file, keep_data: bool = True):
    # shell_command = f'{MRI_EXEC} {MRI_METRICS} --html "{usage_output_file}" -c "{solvio_cmd}" {MRI_OTHER_ARGS}'
    if usage_output_file:
        shell_command = f'solvio__LOG_LEVEL=debug {MRI_EXEC} {MRI_METRICS} --{MRI_OUTPUT_FORMAT} "{usage_output_file}" -c "{solvio_cmd}" {MRI_OTHER_ARGS}'
    else:
        shell_command = f'{MRI_EXEC} {MRI_METRICS} -c "{solvio_cmd}" -d 30'
...
def load_time_simple_reload(init_snapshot) -> None:
    """
    Case when we re-start Solvio using existing storage/ folder.
    ...
    cmd_under_test = f'{solvio_EXEC} --snapshot {init_snapshot} --force-snapshot'
    ...
    # cmd_under_test = f'RUST_LOG=debug {solvio_EXEC}'
    cmd_under_test = solvio_EXEC
