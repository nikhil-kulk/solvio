...
def create_collection(solvio_host, collection_name, collection_json):
    resp = requests.put(
        f"{solvio_host}/collections/{collection_name}?timeout=60", json=collection_json)
    if resp.status_code != 200:
        print(f"Collection creation failed with response body:\n{resp.json()}")
        exit(-1)


def update_collection(solvio_host, collection_name, collection_json: dict):
    resp = requests.patch(
        f"{solvio_host}/collections/{collection_name}?timeout=60", json=collection_json)
    if resp.status_code != 200:
...
def wait_for_status(solvio_host, collection_name, status: str):
    curr_status = ""
    for i in range(30):
        resp = requests.get(
            f"{solvio_host}/collections/{collection_name}")
...
def insert_points(solvio_host, collection_name, batch_json, quit_on_ood: bool = False):
    resp = requests.put(
...
def insert_points_and_search(solvio_host, collection_name, points_amount):
    collection_json = {
        "vectors": {
            "size": VECTOR_SIZE,
            "distance": "Cosine"
    ...
    create_collection(solvio_host, collection_name, collection_json)
...
def insert_points_then_index(solvio_host, collection_name, points_amount):
    """
    ...
    create_collection(solvio_host, collection_name, collection_json)
    for points_batch in generate_points(points_amount):
        res = insert_points(solvio_host, collection_name, points_batch, quit_on_ood=True)
        if res == "ood":
            break
    ...
    update_collection(solvio_host, collection_name, collection_json)
    wait_for_status(solvio_host, collection_name, "yellow")
    wait_for_status(solvio_host, collection_name, "green")
    search_point(solvio_host, collection_name)
...
def main():
    ...
    if args.test_item == "search":
        ...
        insert_points_and_search(solvio_host, args.collection_name, args.points_amount)
    elif args.test_item == "indexing":
        print("run indexing test")
        insert_points_then_index(solvio_host, args.collection_name, args.points_amount)
    else:
