...
def create_collection(solvio_host, collection_name):
    resp = requests.put(
        f"{solvio_host}/collections/{collection_name}?timeout=60", json={
            "vectors": {
                "size": 4,
                "distance": "Cosine"
            }
...
def insert_points(solvio_host, collection_name, batch_json):
    resp = requests.put(
