...
declare OOM_CONTAINER_NAME=solvio-oom
...
docker run \
    ...
    -v $PWD/storage:/solvio/storage \
    -e solvio__STORAGE__HANDLE_COLLECTION_LOAD_ERRORS=true \
...
declare container && container=$(
    docker run --rm -d \
        ...
        -v $PWD/storage:/solvio/storage \
        -e solvio__STORAGE__HANDLE_COLLECTION_LOAD_ERRORS=true \
        -e solvio_ALLOW_RECOVERY_MODE=true \
        $DOCKER_IMAGE_NAME
)
...
declare RECOVERY_MODE_MSG='Solvio is loaded in recovery mode'
