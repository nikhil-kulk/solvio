...
from .settings import solvio_HOST, SCHEMA, solvio_HOST_HEADERS
...
def request_with_validation(
    ...
    response = action(
        url=get_api_string(solvio_HOST, api, path_params),
        params=query_params,
        json=body,
    ...
    try:
        ...
        msg = (
            ...
            f"Request was:\n"
            f"Method: {method}\n"
            f"URL: {get_api_string(solvio_HOST, api, path_params)}\n"
            f"Query params: {query_params}\n"
            f"Headers: {solvio_host_headers()}\n"
            f"Body: {body}\n"
        )
        warnings.warn(msg)
        raise
...
# https://github.com/solvio/solvio-client/blob/d18cb1702f4cf8155766c7b32d1e4a68af11cd6a/solvio_client/hybrid/fusion.py#L6C1-L31C25
...
@lru_cache
def solvio_host_headers():
    headers = json.loads(solvio_HOST_HEADERS)
    return headers