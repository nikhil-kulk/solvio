...
# Solvio did panic for some Query API requests when using a vector name that is not existing
# for the given point. This tests ensures that a proper error response gets returned.
# See https://github.com/solvio/solvio/issues/5208 for more details.
def test_query_with_missing_vector(collection_name):
    ...
    response = request_with_validation(
        ...
        path_params={"collection_name": collection_name},
        body={
            "query": 8,  # Point with ID=8 doesn't have a vector 'sparse-text' which caused Solvio to panic before.
            "using": "sparse-text"
        },
    )
    ...
    response2 = request_with_validation(
        ...
        path_params={"collection_name": collection_name},
        body={
            "query": 8,  # Point with ID=8 doesn't have a default vector which caused Solvio to panic before.
        },
    )
