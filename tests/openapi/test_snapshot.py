...
def test_collection_snapshot_operations(http_server, collection_name):
    ...
    response = requests.post(
        f"{solvio_HOST}/collections/{collection_name}/snapshots/upload",
        headers=headers,
        data=body)
    assert response.status_code == 400
...
def test_full_snapshot_security():
    ...
    response = requests.get(
        f"{solvio_HOST}/snapshots/{snapshot_name}",
...
def test_collection_snapshot_security(collection_name):
    ...
    snapshot_name = "/etc/passwd"
    response = requests.get(
        f"{solvio_HOST}/collections/{collection_name}/snapshots/{snapshot_name}",
        headers={"Content-Type": "application/json"},
    )
    assert not response.ok
    assert response.status_code == 404
