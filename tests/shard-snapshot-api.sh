...
declare solvio_HOST="${solvio_HOST:-localhost}"

declare solvio_HTTP_PORT="${solvio_HTTP_PORT:-6333}"
declare solvio_GRPC_PORT="${solvio_GRPC_PORT:-6334}"
...
# When running Solvio in Docker on macOS, use `http://host.docker.internal:8080`
...
function load-solvio-status {
	if [[ ! $CLUSTER ]]
	then
		declare STATUS ; STATUS="$(curl-ok "http://$solvio_HOST:$solvio_HTTP_PORT/cluster" | jq -r .result.status)"
		...
		case "$STATUS" in
			...
			*)
				echo "load-solvio-status: invalid cluster status $STATUS" >&2
				return 1
			;;
		esac
	fi
...
function fixture-with-points {
	...
	then
		...
		then
			"$BFB" \
				--uri "http://$solvio_HOST:$solvio_GRPC_PORT" \
...
function url {
	...
	declare URL="$solvio_HOST:$solvio_HTTP_PORT"
