...
def drop_collection(name: str):
    ...
    requests.delete(f"http://{solvio_HOST}/collections/{name}")
...
def create_collection(name: str, quantization_config: dict = None):
    ...
    response = requests.put(
        f"http://{solvio_HOST}/collections/{name}",
...
def create_payload_indexes(name: str):
    ...
    response = requests.put(
        f"http://{solvio_HOST}/collections/{name}/index",
...
def upload_points(name: str):
    ...
    response = requests.put(
        f"http://{solvio_HOST}/collections/{name}/points?wait=true",
        headers={"Content-Type": "application/json"},
        json={
            "points": [
                {
...
def basic_retrieve(name: str):
    response = requests.get(
        f"http://{solvio_HOST}/collections/{name}/points/2",
        headers={"Content-Type": "application/json"},
    )
    assert response.ok

    response = requests.post(
        f"http://{solvio_HOST}/collections/{name}/points",
        headers={"Content-Type": "application/json"},
        json={"ids": [1, 2]},
    )
    assert response.ok
