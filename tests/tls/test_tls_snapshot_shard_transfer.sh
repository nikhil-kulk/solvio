...
function main {
	...
	docker buildx build --load ../../ --tag=solvio_tls --build-arg=PROFILE=ci
	...
	cat "$SELF" | docker container exec -i tls-solvio_node_1-1 bash -x -s test https://node1.solvio:6333
}
